        ...
        private static async void StartServer()
        {
            try
            {
                ...
                mqttServer.ClientSubscribedTopicHandler =
                    new MqttServerClientSubscribedTopicHandlerDelegate((Action<MqttServerClientSubscribedTopicEventArgs>)SubScribedTopic);
                // 设置消息退订通知
                mqttServer.ClientUnsubscribedTopicHandler =
                    new MqttServerClientUnsubscribedTopicHandlerDelegate((Action<MqttServerClientUnsubscribedTopicEventArgs>)UnScribedTopic);
                // 设置消息处理程序
                mqttServer.UseApplicationMessageReceivedHandler((Action<MqttApplicationMessageReceivedEventArgs>)MessageReceived);
                // 设置客户端连接成功后的处理程序
